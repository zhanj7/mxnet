 ...
 * \file proposal_target_sv-inl.h
 * \brief C++ version proposal target
 * \author Yuntao Chen, Pengfei Chen
 */
#ifndef MXNET_OPERATOR_PROPOSAL_TARGET_SV_INL_H_
#define MXNET_OPERATOR_PROPOSAL_TARGET_SV_INL_H_
...
namespace proposal_target_sv_enum {
enum ProposalTargetSVInputs {kRois, kGtBboxes};
enum ProposalTargetSVOutputs {kRoiOutput, kLabel, kBboxTarget, kBboxWeight};
}

struct ProposalTargetSVParam : public dmlc::Parameter<ProposalTargetSVParam> {
  index_t num_classes;
  index_t image_rois;
  float fg_fraction;
  float fg_thresh;
  ...
  DMLC_DECLARE_PARAMETER(ProposalTargetSVParam) {
...
class ProposalTargetSVOp : public Operator {
 public:
  explicit ProposalTargetSVOp(ProposalTargetSVParam param) {
    this->param_ = param;
  }
  ...
  virtual void Forward(const OpContext &ctx,
    ...
    CHECK_EQ(req.size(), 4);
    CHECK_EQ(req[proposal_target_sv_enum::kRoiOutput], kWriteTo);
    CHECK_EQ(req[proposal_target_sv_enum::kLabel], kWriteTo);
    CHECK_EQ(req[proposal_target_sv_enum::kBboxTarget], kWriteTo);
    CHECK_EQ(req[proposal_target_sv_enum::kBboxWeight], kWriteTo);
    
    Stream<xpu> *s = ctx.get_stream<xpu>();
    const index_t num_image         = in_data[proposal_target_sv_enum::kRois].shape_[0];
    const index_t image_rois        = param_.image_rois;
    const index_t num_roi           = in_data[proposal_target_sv_enum::kRois].Size() / (num_image * 4);
    const index_t num_gtbbox        = in_data[proposal_target_sv_enum::kGtBboxes].Size() / (num_image * 5);

    Tensor<xpu, 3, DType> xpu_rois      = in_data[proposal_target_sv_enum::kRois].
                                          get_with_shape<xpu, 3, DType>(Shape3(num_image, num_roi, 4), s);
    Tensor<xpu, 3, DType> xpu_gt_bboxes = in_data[proposal_target_sv_enum::kGtBboxes].
    ...
    Tensor<xpu, 3, DType> xpu_output_rois  = out_data[proposal_target_sv_enum::kRoiOutput].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, 4), s);
    Tensor<xpu, 2, DType> xpu_labels       = out_data[proposal_target_sv_enum::kLabel].
                                             get_with_shape<xpu, 2, DType>(Shape2(num_image, image_rois), s);
    Tensor<xpu, 3, DType> xpu_bbox_targets = out_data[proposal_target_sv_enum::kBboxTarget].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, param_.num_classes * 4), s);
    Tensor<xpu, 3, DType> xpu_bbox_weights = out_data[proposal_target_sv_enum::kBboxWeight].
                                             get_with_shape<xpu, 3, DType>(Shape3(num_image, image_rois, param_.num_classes * 4), s);
  ...
  virtual void Backward(const OpContext &ctx,
    ...
    Stream<xpu> *s = ctx.get_stream<xpu>();
    Tensor<xpu, 3, DType> rois      = in_grad[proposal_target_sv_enum::kRois].get<xpu, 3, DType>(s);
    Tensor<xpu, 3, DType> gt_bboxes = in_grad[proposal_target_sv_enum::kGtBboxes].get<xpu, 3, DType>(s);
 ...
 private:
  ProposalTargetSVParam param_;
};  // class ProposalTargetSVOp

template<typename xpu>
Operator *CreateOp(ProposalTargetSVParam param, int dtype);

#if DMLC_USE_CXX11
class ProposalTargetSVProp : public OperatorProperty {
 public:
  ...
  bool InferShape(std::vector<TShape> *in_shape,
    ...
    const TShape &dshape = in_shape->at(proposal_target_sv_enum::kRois);
  ...
  std::string TypeString() const override {
    return "ProposalTargetSV";
  }

  OperatorProperty *Copy() const override {
    auto ptr = new ProposalTargetSVProp();
    ptr->param_ = param_;
    return ptr;
  }
 ...
 private:
  ProposalTargetSVParam param_;
};  // class ProposalTargetSVProp
#endif  // DMLC_USE_CXX11

}  // namespace op
}  // namespace mxnet

#endif  // MXNET_OPERATOR_PROPOSAL_TARGET_SV_INL_H_
