...
template<>
Operator *CreateOp<cpu>(ProposalTargetSVParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalTargetSVOp<cpu, DType>(param);
  })
  return op;
}

template<>
Operator *CreateOp<gpu>(ProposalTargetSVParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new ProposalTargetSVOp<gpu, DType>(param);
  })
  return op;
}

Operator *ProposalTargetSVProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(ProposalTargetSVParam);

MXNET_REGISTER_OP_PROPERTY(ProposalTargetSV, ProposalTargetSVProp)
.describe("C++ version proposal target")
.add_argument("rois", "Symbol", "rois")
.add_argument("gt_boxes", "Symbol", "gtboxes")
.add_arguments(ProposalTargetSVParam::__FIELDS__());
